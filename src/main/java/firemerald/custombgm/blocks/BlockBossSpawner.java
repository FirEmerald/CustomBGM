...
public class BlockBossSpawner extends BlockContainer
{
    ...
    public void breakBlock(World worldIn, BlockPos pos, IBlockState state)
    {
    	...
    	if (tile instanceof TileEntityBossSpawner) ForgeChunkManager.releaseTicket(((TileEntityBossSpawner) tile).getTicket());
    ...
    {
        TileEntity tileentity = worldIn.getTileEntity(pos);
        return tileentity instanceof TileEntityBossSpawner ? ((TileEntityBossSpawner) tileentity).getLevel() : 0;
    }
	...
	public void addInformation(ItemStack stack, World world, List<String> list, ITooltipFlag flag)
	{
    	...
    	if (root != null && root.hasKey("BlockEntityTag", 10))
    	{
    		NBTTagCompound blockTag = root.getCompoundTag("BlockEntityTag");
    		if (blockTag.hasKey("levelOnActive", 99)) powerOnSpawned = blockTag.getByte("levelOnActive");
    		if (blockTag.hasKey("levelOnKilled", 99)) powerOnKilled = blockTag.getByte("levelOnKilled");
    	}
		list.add(Translator.format("lore.custombgm.boss_spawner", powerOnSpawned, powerOnKilled));
